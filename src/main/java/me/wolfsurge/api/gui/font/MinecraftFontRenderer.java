...
import me.wolfsurge.api.gui.font.CFont;
...
extends CFont {
    ...
    public int drawStringWithShadow(String text, double x2, float y2, int color) {
        float shadowWidth = this.drawString(text, x2 + CustomFont.SHADOW_SPACING.getDoubleValue(), (double)y2 + CustomFont.SHADOW_SPACING.getDoubleValue(), color, true, 8.3f);
    ...
    public void drawRainbowStringWithShadow(String text, double x2, float y2, int color, float speed, float saturation, int seperation) {
        int count = text.length() * seperation;
        int off = 0;
        String chars = "";
        for (char c : text.toCharArray()) {
            this.drawStringWithShadow(String.valueOf(c), x2 + (double)off, y2, ColorUtil.rainbowWave(speed, saturation, 1.0f, count));
