...
import me.wolfsurge.mixin.mixins.MixinRenderer;
...
extends MixinRenderer<EntityItem> {
    ...
    private int transformModelCount(EntityItem itemIn, double x, double y, double z, float p_177077_8_, IBakedModel p_177077_9_) {
        if (Xeno.moduleManager.getModule("ItemPhysics").isEnabled()) {
    ...
    public void doRender(EntityItem entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (Xeno.moduleManager.getModule("ItemPhysics").isEnabled()) {
            ...
            if ((itemstack = (entityitem = entity).getItem()).getItem() != null) {
                ...
                int i = this.transformModelCount(entityitem, x, y, z, partialTicks, ibakedmodel);
                ...
                for (int j = 0; j < i; ++j) {
                    if (ibakedmodel.isAmbientOcclusion()) {
                        GlStateManager.pushMatrix();
                        GlStateManager.scale((double)ItemPhysics.scale.getDoubleValue(), (double)ItemPhysics.scale.getDoubleValue(), (double)ItemPhysics.scale.getDoubleValue());
                        if (Xeno.moduleManager.getModule("ESP").isEnabled() && ESP.items.isEnabled()) {
                            ...
                            this.itemRenderer.renderItem(itemstack, ibakedmodel);
                            OutlineUtils.renderFour(ESP.itemColour.getColor());
                            this.itemRenderer.renderItem(itemstack, ibakedmodel);
                            OutlineUtils.renderFive();
                            OutlineUtils.setColor(ESP.itemColour.getColor());
                        } else {
                            this.itemRenderer.renderItem(itemstack, ibakedmodel);
                        }
