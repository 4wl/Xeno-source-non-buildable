...
extends Theme {
    ...
    public void drawKeybind(KeybindSetting op, double offset, Button parent, boolean binding, int mouseX, int mouseY) {
        TextUtil.drawClickGuiString(binding ? "Press a key..." : op.name + ": " + Keyboard.getKeyName((int)parent.mod.keyCode.getKeyCode()), parent.parent.getX() + 6, (float)((double)parent.parent.getY() + offset + 1.0), parent.parent.getX() + parent.parent.getWidth() / 2, parent.parent.getX() + parent.parent.getWidth(), -1);
    }
            ...
            if (!components.isEmpty()) {
                ...
                for (Component component : components) {
                    ...
                    if (ClickGuiModule.closedButtonOutline.enabled) {
                        RenderUtils2D.drawRoundedOutline(x, (double)y + button.offset + 2.5, ClickGuiVariables.frameWidth, button.open ? (double)modLength + 0.5 : (double)ClickGuiVariables.buttonBarHeight - 1.5, ClickGuiVariables.frameRoundedRadius, 2.0, ClickGui.color);
                    } else if (button.open) {
    ...
    public void drawButton(Frame parent, Module mod, ArrayList<Component> subcomponents, boolean open, double offset, int mouseX, int mouseY, boolean hovered) {
        TextUtil.drawClickGuiString(mod.getName(), parent.getX() + 2, (float)((double)parent.getY() + offset + 2.5), parent.getX() + parent.getWidth() / 2, parent.getX() + parent.getWidth(), mod.isEnabled() ? ClickGui.color : -1);
