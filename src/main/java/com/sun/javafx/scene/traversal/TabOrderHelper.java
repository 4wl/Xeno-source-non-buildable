...
final class TabOrderHelper {
    ...
    public static Node getFirstTargetNode(Parent parent) {
        ...
        for (Node node : observableList) {
            ...
            if (TabOrderHelper.isDisabledOrInvisible(node)) continue;
            ParentTraversalEngine parentTraversalEngine3 = parentTraversalEngine2 = node instanceof Parent ? ((Parent)node).getImpl_traversalEngine() : null;
    ...
    public static Node getLastTargetNode(Parent parent) {
        ...
        for (int i = observableList.size() - 1; i >= 0; --i) {
            ...
            ParentTraversalEngine parentTraversalEngine2 = node = node2 instanceof Parent ? ((Parent)node2).getImpl_traversalEngine() : null;
