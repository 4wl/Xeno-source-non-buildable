...
extends ParsedValue<V, T> {
    ...
    public final void writeBinary(DataOutputStream dataOutputStream, StringStore stringStore) throws IOException {
        ...
        if (this.value instanceof ParsedValue) {
            ...
            throw new InternalError("cannot writeBinary " + (Object)((Object)this));
        }
    }
    ...
    public static ParsedValueImpl readBinary(int n, DataInputStream dataInputStream, String[] arrstring) throws IOException {
        ...
        if (by == 3) {
            ...
            ParsedValueImpl[][] arrparsedValueImpl = (n3 = dataInputStream.readInt()) > 0 ? new ParsedValueImpl[n3][0] : (ParsedValueImpl[][])null;
            for (int i = 0; i < n3; ++i) {
                int n4;
                if (n >= 4) {
                    dataInputStream.readByte();
