...
class OS {
    ...
    static final IDWriteFactory DWriteCreateFactory(int n) {
        long l = OS._DWriteCreateFactory(n);
        return l != 0L ? new IDWriteFactory(l) : null;
    }

    private static final native long _D2D1CreateFactory(int var0);

    static final ID2D1Factory D2D1CreateFactory(int n) {
        long l = OS._D2D1CreateFactory(n);
        return l != 0L ? new ID2D1Factory(l) : null;
    }

    private static final native long _WICCreateImagingFactory();

    static final IWICImagingFactory WICCreateImagingFactory() {
        long l = OS._WICCreateImagingFactory();
        return l != 0L ? new IWICImagingFactory(l) : null;
    }

    private static final native long _NewJFXTextAnalysisSink(char[] var0, int var1, int var2, char[] var3, int var4, long var5);

    static final JFXTextAnalysisSink NewJFXTextAnalysisSink(char[] arrc, int n, int n2, String string, int n3) {
        long l = OS._NewJFXTextAnalysisSink(arrc, n, n2, (string + '\u0000').toCharArray(), n3, 0L);
        return l != 0L ? new JFXTextAnalysisSink(l) : null;
    }

    private static final native long _NewJFXTextRenderer();

    static final JFXTextRenderer NewJFXTextRenderer() {
        long l = OS._NewJFXTextRenderer();
        return l != 0L ? new JFXTextRenderer(l) : null;
    }
