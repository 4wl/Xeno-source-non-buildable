...
implements FontFactory {
    ...
    private PrismFontFile createFontResource(String string, int n) {
        return this.createFontResource(string, n, true, false, false, false);
    }

    private PrismFontFile createFontResource(String string, int n, boolean bl, boolean bl2, boolean bl3, boolean bl4) {
        ...
        try {
            prismFontFile = this.createFontFile(null, string, n, bl, bl2, bl3, bl4);
            if (bl) {
    ...
    private PrismFontFile createFontResource(String string, String string2, boolean bl, boolean bl2, boolean bl3, boolean bl4) {
        ...
        if (string3.endsWith(".ttc")) {
            ...
            } while (++n < prismFontFile.getFontCount());
    ...
    public synchronized FontResource getFontResource(String string, boolean bl, boolean bl2, boolean bl3) {
        ...
        if (isWindows && (string2 = WindowsFontMap.findFontFile(string22, n = (bl ? 1 : 0) + (bl2 ? 2 : 0))) != null && (object3 = this.createFontResource(null, string2)) != null) {
     ...
     */
    @Override
    public PGFont loadEmbeddedFont(String string, InputStream inputStream, float f, boolean bl) {
        if (!this.hasPermission()) {
            return this.createFont("System Regular", f);
        }
        if (FontFileWriter.hasTempPermission()) {
            return this.loadEmbeddedFont0(string, inputStream, f, bl);
        }
        ...
        try {
            ...
            PGFont pGFont = this.loadEmbeddedFont0(string, inputStream, f, bl);
            return pGFont;
        }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private PGFont loadEmbeddedFont0(String string, InputStream inputStream, float f, boolean bl) {
        PrismFontFile prismFontFile = null;
        ...
        try {
            ...
            while ((n = inputStream.read(arrby)) >= 0) {
                fontFileWriter.writeBytes(arrby, 0, n);
            }
        ...
        if (prismFontFile != null) {
            ...
            return new PrismFont(prismFontFile, prismFontFile.getFullName(), f);
        }
        return null;
    }

    @Override
    public PGFont loadEmbeddedFont(String string, String string2, float f, boolean bl) {
        if (!this.hasPermission()) {
            return this.createFont("System Regular", f);
        }
        this.addFileCloserHook();
    ...
    public static float getSystemFontSize() {
        if (systemFontSize == -1.0f) {
            if (isWindows) {
                float f = Screen.getMainScreen().getUIScale();
                systemFontSize = (float)PrismFontFactory.getSystemFontSizeNative() / f;
            } else if (isMacOSX || isIOS) {
