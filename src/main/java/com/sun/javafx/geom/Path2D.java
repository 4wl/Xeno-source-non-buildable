...
implements PathConsumer2D {
    ...
    void needRoom(boolean bl, int n) {
        ...
        } else if (this.numTypes >= n3) {
            n2 = n3;
            if (n2 > 500) {
                n2 = 500;
            }
            this.pointTypes = Path2D.copyOf(this.pointTypes, n3 + n2);
        }
        n3 = this.floatCoords.length;
        if (this.numCoords + n > n3) {
            n2 = n3;
            if (n2 > 1000) {
                n2 = 1000;
            }
            ...
            this.floatCoords = Path2D.copyOf(this.floatCoords, n3 + n2);
        }
    }
    ...
    static byte[] copyOf(byte[] arrby, int n) {
        byte[] arrby2 = new byte[n];
        System.arraycopy(arrby, 0, arrby2, 0, Math.min(arrby.length, n));
        return arrby2;
    }

    static float[] copyOf(float[] arrf, int n) {
        float[] arrf2 = new float[n];
        System.arraycopy(arrf, 0, arrf2, 0, Math.min(arrf.length, n));
        return arrf2;
    }
