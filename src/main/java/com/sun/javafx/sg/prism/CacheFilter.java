...
public class CacheFilter {
    ...
    private boolean needToRenderCache(BaseTransform baseTransform, double[] arrd, float f) {
        ...
        if (this.lastXDelta != 0.0 || this.lastYDelta != 0.0) {
            ...
            if (this.scrollCacheState == ScrollCacheState.CHECKING_PRECONDITIONS) {
                if (this.impl_scrollCacheCapable() && this.isXformScrollCacheCapable(arrd)) {
        ...
        double d3 = arrd[2];
        if (this.scaleHint) {
            if (this.cachedScaleX < (double)f || this.cachedScaleY < (double)f) {
                return true;
            }
            if (this.rotateHint) {
                return false;
    ...
    double[] unmatrix(BaseTransform baseTransform) {
        ...
        double d = Math.signum(arrarrd[0][0]);
        double d2 = Math.signum(arrarrd[1][1]);
    ...
    void render(Graphics graphics) {
        ...
        if (this.needToRenderCache(baseTransform, arrd, f = graphics.getAssociatedScreen().getRenderScale())) {
            ...
            if (this.scaleHint) {
                this.cachedScaleX = Math.max((double)f, arrd[0]);
            ...
            this.cachedImageData = this.impl_createImageData(prFilterContext, this.cacheBounds);
        ...
        if (object == null) {
            ...
            this.impl_renderNodeToScreen(graphics);
        } else {
            double d = baseTransform.getMxt();
            double d2 = baseTransform.getMyt();
            this.impl_renderCacheToScreen(graphics, (Filterable)object, d, d2);
            object.unlock();
        }
    }

    ImageData impl_createImageData(FilterContext filterContext, Rectangle rectangle) {
        Filterable filterable;
        try {
    ...
    void impl_renderNodeToCache(ImageData imageData, Rectangle rectangle, BaseTransform baseTransform, Rectangle rectangle2) {
    ...
    void impl_renderNodeToScreen(Object object) {
    ...
    void impl_renderCacheToScreen(Object object, Filterable filterable, double d, double d2) {
        Graphics graphics = (Graphics)object;
    ...
    void impl_moveCacheBy(ImageData imageData, double d, double d2) {
        PrDrawable prDrawable = (PrDrawable)imageData.getUntransformedImage();
    ...
    Rectangle impl_getCacheBounds(Rectangle rectangle, BaseTransform baseTransform) {
