...
extends GlassStage {
    ...
    public void setBounds(float f, float f2, boolean bl, boolean bl2, float f3, float f4, float f5, float f6, float f7, float f8) {
        ...
        float f9 = this.platformWindow.getPlatformScale();
        if (bl || bl2) {
            float f10;
            float f11;
            ...
            if (list.size() > 1) {
                f11 = bl ? f : (float)this.platformWindow.getX();
                f10 = bl2 ? f2 : (float)this.platformWindow.getY();
                float f12 = (float)screen.getX() + (float)screen.getWidth() / 2.0f - f11;
                float f13 = (float)screen.getY() + (float)screen.getHeight() / 2.0f - f10;
                float f14 = f12 * f12 + f13 * f13;
                for (Screen screen2 : Screen.getScreens()) {
                    f12 = (float)screen2.getX() + (float)screen2.getWidth() / 2.0f - f11;
                    float f15 = f12 * f12 + (f13 = (float)screen2.getY() + (float)screen2.getHeight() / 2.0f - f10) * f13;
                    if (!(f15 < f14)) continue;
                    screen = screen2;
                    f14 = f15;
                }
            }
            f11 = screen == null ? 0.0f : (float)screen.getX();
            f10 = screen == null ? 0.0f : (float)screen.getY();
            n2 = bl ? Math.round(f11 + (f - f11) * f9) : 0;
            n = bl2 ? Math.round(f10 + (f2 - f10) * f9) : 0;
        } else {
            n = 0;
            n2 = 0;
        }
        int n3 = (int)(f3 > 0.0f ? Math.ceil(f3 * f9) : (double)f3);
        int n4 = (int)(f4 > 0.0f ? Math.ceil(f4 * f9) : (double)f4);
        int n5 = (int)(f5 > 0.0f ? Math.ceil(f5 * f9) : (double)f5);
        int n6 = (int)(f6 > 0.0f ? Math.ceil(f6 * f9) : (double)f6);
        this.platformWindow.setBounds(n2, n, bl, bl2, n3, n4, n5, n6, f7, f8);
    }
    ...
    public void setMinimumSize(int n, int n2) {
        float f = this.platformWindow.getPlatformScale();
        n = (int)Math.ceil((float)n * f);
        n2 = (int)Math.ceil((float)n2 * f);
