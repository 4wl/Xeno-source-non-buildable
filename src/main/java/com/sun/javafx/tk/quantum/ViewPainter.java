...
implements Runnable {
    ...
    protected void paintImpl(Graphics graphics) {
        ...
        float f = this.getPixelScaleFactor();
        graphics2.setPixelScaleFactor(f);
        ...
        if (bl && !this.sceneState.getScene().getDepthBuffer()) {
            ...
            n2 = (int)Math.ceil((float)this.height * f);
        ...
        if (n2 > 0) {
            ...
            for (n5 = 0; n5 < n2; ++n5) {
                object = ViewPainter.getRootPath(n5);
                ((NodePath)object).clear();
                this.root.getRenderRoot(ViewPainter.getRootPath(n5), this.dirtyRegionContainer.getDirtyRegion(n5), n5, this.tx, this.projTx);
            }
            if (PulseLogger.PULSE_LOGGING_ENABLED) {
                PulseLogger.addMessage(n2 + " different dirty regions to render");
                for (n5 = 0; n5 < n2; ++n5) {
                    PulseLogger.addMessage("Dirty Region " + n5 + ": " + this.dirtyRegionContainer.getDirtyRegion(n5));
                    PulseLogger.addMessage("Render Root Path " + n5 + ": " + ViewPainter.getRootPath(n5));
                }
            }
            if (PulseLogger.PULSE_LOGGING_ENABLED && PrismSettings.printRenderGraph) {
                ...
                for (int i = 0; i < n2; ++i) {
                    NodePath nodePath;
                    RectBounds rectBounds = this.dirtyRegionContainer.getDirtyRegion(i);
                    if (!(rectBounds.getWidth() > 0.0f) || !(rectBounds.getHeight() > 0.0f) || (nodePath = ViewPainter.getRootPath(i)).isEmpty()) continue;
            ...
            for (n = 0; n < n2; ++n) {
                ...
                if (f != 1.0f) {
                    this.dirtyRect.x = (int)((float)this.dirtyRect.x * f);
                    this.dirtyRect.y = (int)((float)this.dirtyRect.y * f);
                    this.dirtyRect.width = (int)((float)this.dirtyRect.width * f);
                    this.dirtyRect.height = (int)((float)this.dirtyRect.height * f);
                }
    ...
    protected float getPixelScaleFactor() {
        return this.presentable == null ? 1.0f : this.presentable.getPixelScaleFactor();
    }
