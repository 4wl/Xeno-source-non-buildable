    ...
    private boolean updateSkipMeshNormalGeometry(int[] arrn, int[] arrn2) {
        ...
        if (n5 > 0) {
            for (n3 = 0; n3 < n5; ++n3) {
                int n6;
                n2 = (n4 + n3) * 2;
                MeshGeomComp2VB meshGeomComp2VB = this.texCoord2vbMap.get(n2);
                assert (meshGeomComp2VB != null);
                if (meshGeomComp2VB == null) continue;
                int[] arrn3 = meshGeomComp2VB.getLocs();
                int n7 = meshGeomComp2VB.getValidLocs();
                if (arrn3 != null) {
                    for (n6 = 0; n6 < n7; ++n6) {
                        n = arrn3[n6] * 9 + 3;
                        this.vertexBuffer[n] = this.uv[n2];
                        this.vertexBuffer[n + 1] = this.uv[n2 + 1];
                    }
                    continue;
                }
                n6 = meshGeomComp2VB.getLoc();
                n = n6 * 9 + 3;
                this.vertexBuffer[n] = this.uv[n2];
                this.vertexBuffer[n + 1] = this.uv[n2 + 1];
            }
        ...
        if (n2 > 0) {
            for (int i = 0; i < n2; ++i) {
                ...
                n = meshGeomComp2VB.getValidLocs();
                if (arrn4 != null) {
                    for (n9 = 0; n9 < n; ++n9) {
                        n8 = arrn4[n9] * 9;
                        this.vertexBuffer[n8] = this.pos[n10];
                        this.vertexBuffer[n8 + 1] = this.pos[n10 + 1];
                        this.vertexBuffer[n8 + 2] = this.pos[n10 + 2];
                    }
                    continue;
                }
                n9 = meshGeomComp2VB.getLoc();
                n8 = n9 * 9;
                this.vertexBuffer[n8] = this.pos[n10];
                this.vertexBuffer[n8 + 1] = this.pos[n10 + 1];
                this.vertexBuffer[n8 + 2] = this.pos[n10 + 2];
    ...
    private boolean buildSkipMeshNormalGeometry() {
        ...
        for (n2 = 0; n2 < this.nFaces; ++n2) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                int n6 = n + n5;
                int n7 = n6 + 1;
                long l = (long)this.faces[n6] << 32 | (long)this.faces[n7];
                Integer n8 = (Integer)hashMap.get(l);
                if (n8 == null) {
                    n8 = n4 / 9;
                    hashMap.put(l, n8);
                    if (this.vertexBuffer.length <= n4) {
                        float[] arrf = new float[n4 + 90];
                        System.arraycopy(this.vertexBuffer, 0, arrf, 0, this.vertexBuffer.length);
                        this.vertexBuffer = arrf;
                    }
                    int n9 = this.faces[n6] * 3;
                    int n10 = this.faces[n7] * 2;
                    this.vertexBuffer[n4] = this.pos[n9];
                    this.vertexBuffer[n4 + 1] = this.pos[n9 + 1];
                    this.vertexBuffer[n4 + 2] = this.pos[n9 + 2];
                    this.vertexBuffer[n4 + 3] = this.uv[n10];
                    this.vertexBuffer[n4 + 4] = this.uv[n10 + 1];
                    this.vertexBuffer[n4 + 5] = 0.0f;
                    this.vertexBuffer[n4 + 6] = 0.0f;
                    this.vertexBuffer[n4 + 7] = 0.0f;
                    this.vertexBuffer[n4 + 8] = 0.0f;
                    n4 += 9;
                    MeshGeomComp2VB meshGeomComp2VB = this.point2vbMap.get(n9);
                    if (meshGeomComp2VB == null) {
                        meshGeomComp2VB = new MeshGeomComp2VB(n9, n8);
                        this.point2vbMap.put(n9, meshGeomComp2VB);
                    } else {
                        meshGeomComp2VB.addLoc(n8);
                    }
                    MeshGeomComp2VB meshGeomComp2VB2 = this.texCoord2vbMap.get(n10);
                    if (meshGeomComp2VB2 == null) {
                        meshGeomComp2VB2 = new MeshGeomComp2VB(n10, n8);
                        this.texCoord2vbMap.put(n10, meshGeomComp2VB2);
                    } else {
                        meshGeomComp2VB2.addLoc(n8);
                    }
                }
                this.indexBuffer[n3++] = n8;
            }
        }
        this.numberOfVertices = n4 / 9;
    ...
    private boolean doBuildPNTGeometry(float[] arrf, float[] arrf2, float[] arrf3, int[] arrn) {
        ...
        for (n2 = 0; n2 < n11; ++n2) {
            ...
            for (n15 = 0; n15 < 3; ++n15) {
                ...
                int n21 = arrn[n18] * 3;
                int n22 = arrn[n19] * 2;
                meshTempState.triPointIndex[n15] = n16;
                meshTempState.triTexCoordIndex[n15] = n22;
                meshTempState.triVerts[n15] = n13 / 9;
                this.vertexBuffer[n13] = arrf[n16];
                this.vertexBuffer[n13 + 1] = arrf[n16 + 1];
                this.vertexBuffer[n13 + 2] = arrf[n16 + 2];
                this.vertexBuffer[n13 + 3] = arrf3[n22];
                this.vertexBuffer[n13 + 4] = arrf3[n22 + 1];
                int n23 = meshTempState.triVerts[n15] * 3;
                this.cachedNormals[n23] = arrf2[n21];
                this.cachedNormals[n23 + 1] = arrf2[n21 + 1];
                this.cachedNormals[n23 + 2] = arrf2[n21 + 2];
                n13 += 9;
                MeshGeomComp2VB meshGeomComp2VB = this.point2vbMap.get(n16);
                if (meshGeomComp2VB == null) {
                    meshGeomComp2VB = new MeshGeomComp2VB(n16, n20);
                    this.point2vbMap.put(n16, meshGeomComp2VB);
                } else {
                    meshGeomComp2VB.addLoc(n20);
                }
                MeshGeomComp2VB meshGeomComp2VB2 = this.normal2vbMap.get(n21);
                if (meshGeomComp2VB2 == null) {
                    meshGeomComp2VB2 = new MeshGeomComp2VB(n21, n20);
                    this.normal2vbMap.put(n21, meshGeomComp2VB2);
                } else {
                    meshGeomComp2VB2.addLoc(n20);
                }
                MeshGeomComp2VB meshGeomComp2VB3 = this.texCoord2vbMap.get(n22);
                if (meshGeomComp2VB3 == null) {
                    meshGeomComp2VB3 = new MeshGeomComp2VB(n22, n20);
                    this.texCoord2vbMap.put(n22, meshGeomComp2VB3);
                } else {
                    meshGeomComp2VB3.addLoc(n20);
                }
                this.indexBuffer[n12++] = n20;
            }
            for (n15 = 0; n15 < 3; ++n15) {
    ...
    private void computeTBNormal(MeshVertex[] arrmeshVertex, MeshVertex[] arrmeshVertex2, int[] arrn) {
        ...
        for (int i = 0; i < this.nFaces; ++i) {
            ...
            if (MeshUtil.isDeadFace(arrn3) && PlatformLogger.getLogger(string).isLoggable(PlatformLogger.Level.FINE)) {
                PlatformLogger.getLogger(string).fine("Dead face [" + arrn3[0] + ", " + arrn3[1] + ", " + arrn3[2] + "] @ face group " + i + "; nEmptyFaces = " + ++n);
            }
            for (n3 = 0; n3 < 3; ++n3) {
