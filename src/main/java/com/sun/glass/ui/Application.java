...
public abstract class Application {
    ...
    private boolean notifyThemeChanged(String string) {
        EventHandler eventHandler = this.getEventHandler();
        if (eventHandler != null) {
    ...
    private void notifyOpenFiles(String[] arrstring) {
    ...
    static Pixels createPixels(int n, int n2, int[] arrn, float f) {
        return Application.GetApplication().createPixels(n, n2, IntBuffer.wrap(arrn), f);
    }
    ...
    static float getScaleFactor(int n, int n2, int n3, int n4) {
        ...
        for (Screen screen : Screen.getScreens()) {
            ...
            if (n >= n5 + n7 || n + n3 <= n5 || n2 >= n6 + n8 || n2 + n4 <= n6 || !(f < screen.getRenderScale())) continue;
